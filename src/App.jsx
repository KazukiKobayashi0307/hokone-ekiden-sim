...
function simAllEk(entries,secs){
  ...
  for(let si=0;si<secs.length;si++){
    const s=secs[si];
    const runners=teams.map(t=>{const r=t.ag[s.id]||genR(50,3);const res=cSec(r,s,true);return{team:t,runner:r,baseTime:res.total,ckpts:res.ckpts};});
    /* Initialize per-team running cum and per-section data */
    runners.forEach(rd=>{rd.runCum=rd.team.st.length>0?rd.team.st[rd.team.st.length-1].cum:0;rd.sectionTime=0;rd.prevPos=0;});
    /* Process each checkpoint: apply penalty based on gap to current leader at that checkpoint */
    const nCk=runners[0].ckpts.length;
    for(let ci=0;ci<nCk;ci++){
      /* Find leader at this point (smallest runCum) */
      ...
      runners.forEach(rd=>{
        ...
        const segT=Math.max(0.5,rd.ckpts[ci].time+(penPerKm-packBonus)*segLen);
        rd.runCum+=segT;rd.sectionTime+=segT;
        rd.team.ckpts.push({totalDist:totalDistSoFar+rd.ckpts[ci].pos,cum:rd.runCum});
        rd.prevPos=rd.ckpts[ci].pos;
      });
    }
    /* Push final section result */
    runners.forEach(rd=>{const finalTime=Math.max(1,Math.round(rd.sectionTime));rd.team.st.push({time:finalTime,cum:rd.runCum,runner:rd.runner.name});});
    totalDistSoFar+=s.km;
  }
...
function autoDraft(rivals,myRunners,mySRes,myHist,rivalHists,myFac,gameYr){
  ...
  const top15Names=new Set(sortedByScore.slice(0,15).map(t=>t.name));
  ...
  for(let pick=0;pick<480;pick++){
    ...
    let elTeams;
    if(pick<20)elTeams=teams.filter(t=>top10Names.has(t.name)&&t.picks.length<12);
    else if(pick<40)elTeams=teams.filter(t=>top15Names.has(t.name)&&t.picks.length<12);
...
export default function Game(){
  ...
  const upgradeFac=useCallback(key=>{const cur=fac[key];if(cur>=10)return;const costs=[0,1,3,5,7,10,13,16,20,24,30];const cost=costs[cur+1]||30;if(prestige<cost)return;setPrestige(p=>p-cost);setFac(p=>({...p,[key]:cur+1}));},[fac,prestige]);

  /* ═══ RENDERS ═══ */
  if(ph==="title")return(<><style>{CSS}</style><div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(180deg,#fff,#f0ede6)",textAlign:"center",padding:24}}><div style={{fontSize:13,letterSpacing:10,color:"var(--ac)",fontWeight:800,marginBottom:20}}>HAKONE EKIDEN SIMULATOR</div><h1 className="sf" style={{fontSize:"clamp(34px,7vw,56px)",fontWeight:800}}>箱根駅伝</h1><div className="sf" style={{fontSize:"clamp(16px,3vw,22px)",color:"var(--t2)",marginTop:6,marginBottom:10}}>監督キャリアモード</div><div style={{fontSize:13,color:"var(--t2)",marginBottom:28,lineHeight:1.9,maxWidth:420}}>複数年育成。オートセーブ。<br/>箱根駅伝1月開催。出雲＝前年箱根10位以内。</div><div style={{display:"flex",alignItems:"center",gap:4,marginBottom:14}}><input value={tn} onChange={e=>setTn(e.target.value)} className="sf" style={{background:"#fff",border:"2px solid var(--bdr)",borderRadius:10,padding:"12px 16px",color:"var(--t)",fontSize:18,textAlign:"right",width:160,outline:"none"}} placeholder="箱根"/><span className="sf" style={{fontSize:18,fontWeight:700}}>大学</span></div><div style={{marginBottom:20}}><div style={{fontSize:12,color:"var(--t2)",marginBottom:6}}>スクールカラー</div><div style={{display:"flex",flexWrap:"wrap",gap:5,maxWidth:340,justifyContent:"center"}}>{["#C0392B","#E67E22","#F39C12","#16A085","#27AE60","#2980B9","#8E44AD","#2C3E50","#D35400","#E74C3C","#1ABC9C","#3498DB","#9B59B6","#34495E","#7F8C8D","#95A5A6","#006633","#7B2D8B","#8B2252","#D32F2F","#5B3C8F","#1C2841","#0055AA","#2D1A4E","#0046AD","#DAA520","#2E7D32","#7B1FA2","#1976D2","#5BC0EB"].map(c=>(<button key={c} onClick={()=>setTc(c)} style={{width:30,height:30,borderRadius:"50%",background:c,border:tc===c?"3px solid #000":"2px solid var(--bdr)",cursor:"pointer"}}/>))}</div></div><div style={{display:"flex",gap:10}}><button onClick={startNew} style={{padding:"14px 40px",borderRadius:50,background:"var(--ac)",color:"#fff",fontWeight:800,fontSize:16}}>新規開始</button><button onClick={loadGame} style={{padding:"14px 30px",borderRadius:50,background:"var(--card)",color:"var(--ac)",fontWeight:700,border:"1px solid var(--bdr)"}}>ロード</button><button onClick={()=>setPh("help")} style={{padding:"14px 30px",borderRadius:50,background:"var(--card)",color:"var(--t2)",fontWeight:700,border:"1px solid var(--bdr)"}}>遊び方</button></div></div></>);
  ...
  if(ph==="help")return(<><style>{CSS}</style><div style={{minHeight:"100vh",background:"var(--bg)",padding:14}}><div style={{maxWidth:700,margin:"0 auto"}} className="fi">
    ...
    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      ...
      <div style={{fontSize:13,lineHeight:1.8,color:"var(--t2)"}}>あなたは弱小校の駅伝部監督。複数年かけて選手を育成し、3大駅伝（出雲・全日本・箱根）での優勝を目指します。学生は4年で卒業し、毎年新入生を迎えます。</div>
    </div>

    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      <h3 className="sf" style={{fontSize:18,marginBottom:8,color:"var(--ac)"}}>📅 1年の流れ</h3>
      <div style={{fontSize:13,lineHeight:1.9,color:"var(--t2)"}}>
        <div><b>1月〜3月</b>: 箱根駅伝（1月）→ オフシーズン</div>
        <div><b>4月〜6月</b>: 春シーズン、記録会で5000m/10000mのタイム狙い</div>
        <div><b>7月〜8月</b>: 夏合宿（全選手の能力&コンディションUP）</div>
        <div><b>9月</b>: 全日本予選会 / 出雲駅伝（10月）の最終調整</div>
        <div><b>10月</b>: 出雲駅伝、箱根予選会（ハーフマラソン）</div>
        <div><b>11月</b>: 全日本大学駅伝</div>
        <div><b>12月</b>: 箱根駅伝最終調整、年末に新入生入学確定</div>
    ...
    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      ...
      <div style={{fontSize:13,lineHeight:1.9,color:"var(--t2)"}}>
        ...
        <div><b>安定感</b>: タイムのブレと先頭との差ペナルティ軽減</div>
        <div><b>登坂力</b>: 山登り（5区など）での登り耐性</div>
        <div><b>降坂力</b>: 山下り（6区）でのスピード活用</div>
        <div><b>単独走</b>: 一人で走る区間で活きる</div>
        <div><b>集団走</b>: パックで走る区間で活きる（特に箱根1〜3区）</div>
    ...
    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      ...
      <div style={{fontSize:13,lineHeight:1.9,color:"var(--t2)"}}>
        <div>・各選手は<b>ポテンシャル（最大能力）</b>を持ち、現在能力との差分が「伸びしろ」です</div>
        <div>・選手ごとに<b>成長期</b>(1〜4年)が設定され、その年は成長効率が2倍、前後年は1.3倍になります</div>
        <div>・練習メニュー（バランス/スピード/スタミナ等12種）に応じて伸びる能力が変わります</div>
        <div>・駅伝出場は確定的に成長を促します（記録会も同様に効果あり）</div>
        <div>・施設（練習Lv・コーチLv）が高いほど成長率が上がります</div>
      </div>
    </div>
    ...
    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      ...
      <div style={{fontSize:13,lineHeight:1.9,color:"var(--t2)"}}>
        <div>・練習やレースで<b>疲労</b>が蓄積。疲労が高いと故障率が上がります</div>
        <div>・<b>休養</b>を入れると疲労減&コンディション回復</div>
        <div>・<b>自動休養</b>機能で疲労が閾値を超えた選手は自動で休ませられます</div>
        <div>・<b>医療レベル</b>が高いほど怪我率減&復帰早まる</div>
      </div>
    </div>

    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      <h3 className="sf" style={{fontSize:18,marginBottom:8,color:"var(--ac)"}}>🎓 スカウトポイント</h3>
      <div style={{fontSize:13,lineHeight:1.9,color:"var(--t2)"}}>
        <div>毎年12月の入学で新入生を獲得。スカウトポイントが多いほど良い選手を取れます。獲得方法:</div>
        <div style={{marginTop:6}}>・<b>3大駅伝の好成績</b>（箱根1位+30pt、区間賞+3pt等）</div>
        <div>・<b>28分台ランナー</b>（10000m sub-28）の人数 × 1pt</div>
        <div>・<b>知名度Lv × 5pt</b>を毎年自動加算</div>
        <div>・<b>練習Lv × 2pt + 医療Lv × 2pt + コーチLv × 1pt</b>を毎年加算</div>
        <div style={{marginTop:6}}><b>有力選手</b>: 上位ランクの新入生は強豪校に集まる傾向があります</div>
        <div style={{marginTop:6,fontSize:12,color:"var(--t3)"}}>初期スカウトポイントは年々減衰: 1年目満額→2年目2/3→3年目1/3→4年目以降0</div>
      </div>
    </div>

    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      <h3 className="sf" style={{fontSize:18,marginBottom:8,color:"var(--ac)"}}>🏆 出場権</h3>
      <div style={{fontSize:13,lineHeight:1.9,color:"var(--t2)"}}>
        <div>・<b>出雲駅伝</b>: 前年箱根10位以内（学連選抜は除外、繰り上がり方式）</div>
        <div>・<b>全日本大学駅伝</b>: 前年8位以内シード、不足校は予選会から</div>
        <div>・<b>箱根駅伝</b>: 前年10位以内シード、不足校はハーフ予選会から10校通過</div>
        <div>・<b>関東学連選抜</b>: 箱根本戦不出場校から各1名のエース選手で編成</div>
      </div>
    </div>
    ...
    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      ...
      <div style={{fontSize:13,lineHeight:1.9,color:"var(--t2)"}}>
        ...
        <div>・<b>医療</b>: 怪我率減＆復帰早め</div>
        <div>・<b>コーチ</b>: 成長率UP（最大2倍、練習施設と乗算）</div>
  ...
  if(ph==="settings")return(<><style>{CSS}</style><div style={{minHeight:"100vh",background:"var(--bg)",padding:14}}><div style={{maxWidth:600,margin:"0 auto"}} className="fi">
    ...
    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:8}}>スクールカラー</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:5}}>{["#C0392B","#E67E22","#F39C12","#16A085","#27AE60","#2980B9","#8E44AD","#2C3E50","#D35400","#E74C3C","#1ABC9C","#3498DB","#9B59B6","#34495E","#7F8C8D","#95A5A6","#006633","#7B2D8B","#8B2252","#D32F2F","#5B3C8F","#1C2841","#0055AA","#2D1A4E","#0046AD","#DAA520","#2E7D32","#7B1FA2","#1976D2","#5BC0EB"].map(c=>(<button key={c} onClick={()=>setTc(c)} style={{width:32,height:32,borderRadius:"50%",background:c,border:tc===c?"3px solid #000":"2px solid var(--bdr)",cursor:"pointer"}}/>))}</div>
    </div>
  </div></div></>);
