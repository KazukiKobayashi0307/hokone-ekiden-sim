...
export default function Game(){
  ...
  useEffect(()=>{if(turn>0&&ph!=="title"&&rs.length>0&&rivals.length>0){doSave({tn,tc,gameYear,turn,rs,rivals,log,fac,prestige,hakQ,zenQ,zenQTeams,hakQTeams,sRes,prevHakone,prevZennihon,prevHakoneRanks,prevZennihonRanks,recs,yearHistory,scoutHist,autoRest,autoRestThreshold,hallOfFame,teamRecords,foreignAllowed,univAlias,ekAlias,draftResult,teamCtrl,sectionRecords,playerHOF,proteges,protegeSlots,consecutiveTripleCrowns,_ph:ph,_raceRes:(ph==="ek_race"?raceRes:null),_raceIdx:raceIdx,_curEk:curEk,_asgn:asgn});}},[ph,turn,fac,prestige,rs,rivals,recs,scoutHist,sRes,gameYear,hallOfFame,teamRecords,foreignAllowed,univAlias,ekAlias,draftResult,teamCtrl,sectionRecords,playerHOF,proteges,protegeSlots,consecutiveTripleCrowns,raceRes,raceIdx,curEk,asgn]);
  ...
  if(ph==="main"){const up=[];for(let t=turn+1;t<=Math.min(turn+6,48);t++){if(CAL[t])up.push({turn:t,...CAL[t]});}return(<><style>{CSS}</style><div style={{minHeight:"100vh",background:"var(--bg)",padding:14}}><div style={{maxWidth:700,margin:"0 auto"}}>
    ...
    {up.length>0&&<div style={{marginBottom:8,padding:"6px 12px",borderRadius:8,background:"var(--card)",border:"1px solid var(--bdr)"}}><div style={{fontSize:11,color:"var(--t2)",fontWeight:700}}>📅 予定</div>{up.map((e,i)=>{const displayN=e.eid&&ekAlias[e.eid]?ekAlias[e.eid]:e.t==="zenQ"?(ekAlias.zennihon||"全日本大学駅伝")+"予選会":e.t==="hakQ"?(ekAlias.hakone||"箱根駅伝")+"予選会":e.n;return(<div key={i} style={{fontSize:13,color:e.turn===turn+1?"var(--ac)":"var(--t)",fontWeight:e.turn===turn+1?700:400}}>{moOf(e.turn)}{wkOf(e.turn)}週: {displayN}{e.turn===turn+1?" ← 次":""}</div>);})}</div>}
