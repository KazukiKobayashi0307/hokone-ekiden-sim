...
export default function Game(){
  ...
  const doFreeRace=useCallback(()=>{
    ...
    const trained=rs.map(r=>{
      ...
      const tr=TRS.find(x=>x.id===r.trn)||TRS[0];const fxUse=r.protege?Object.fromEntries(Object.entries(tr.fx).map(([k,v])=>[k,v*1.1])):tr.fx;const ns=applyGrowth(r,fxUse,fac);
  ...
  if(ph==="scholarship"){const COSTS=[30,50,80,130,200];const nextCost=scholarshipSlots<5?COSTS[scholarshipSlots]:null;const totalSpent=COSTS.slice(0,scholarshipSlots).reduce((a,b)=>a+b,0);return(<><style>{CSS}</style><div style={{minHeight:"100vh",background:"linear-gradient(180deg,#fff,#f0ede6)",padding:18}}><div style={{maxWidth:560,margin:"0 auto"}} className="fi">
