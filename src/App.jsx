...
export default function Game(){
  ...
  const runEkCommon=useCallback((isWatch)=>{const secs=ek.sec;const nR=curEk==="izumo"?10:curEk==="zennihon"?15:20;
    ...
    if(!isWatch){const myT=allT.find(t=>t.isMe);
      setRs(p=>p.map(r=>{
        ...
        return{...base,stats:ekGrow(base),fatigue:Math.min(100,base.fatigue+45),condition:Math.max(10,base.condition-20),history:[...(base.history||[]),{ek:ek.name,yr:gameYear,sec:si>=0?secs[si].n:"",time:si>=0?myT.st[si].time:0,secRank:secRk}]};
      }));}
    ...
    if(!isWatch)setSRes(p=>{const np={...p,[curEk]:rank,[curEk+"SecWins"]:secWinners[teamName]||0};
      /* Triple crown check: all 3 ekidens won this same year */
  ...
  if(ph==="help")return(<><style>{CSS}</style><div style={{minHeight:"100vh",background:"var(--bg)",padding:14}}><div style={{maxWidth:700,margin:"0 auto"}} className="fi">
    ...
    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      ...
      <div style={{fontSize:13,lineHeight:1.8,color:"var(--t2)"}}>あなたは大学駅伝部の監督です。選手をスカウトして育成し、3大駅伝（{ekAlias.izumo}・{ekAlias.zennihon}・{ekAlias.hakone}）での優勝を目指します。学生は4年間在籍して卒業し、毎年新しい新入生を迎えます。何年もかけて強豪校を育て上げましょう。タイトル画面では大学を新しく作るか、既存の大学から選んで監督に就任できます。</div>
    </div>
