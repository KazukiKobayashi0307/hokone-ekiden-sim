...
const FOREIGN_NAMES=["ヴィンセント","ジョセフ","ダニエル","リチャード","ステファン","ピーター","サイモン","ベンジャミン","ジョン","ポール","ジェームス","デイヴィッド","アンドリュー","マイケル","ロバート","ウィリアム","チャールズ","ヘンリー","エドワード","フィリップ","オスカー","エマニュエル","パトリック","アレックス","クリストファー","フランシス","ジェレミー","ニコラス","レナード","マーティン"];
const FOREIGN_LAST=["キプチュンバ","キプタヌイ","ロチャム","ンディク","ムワンギ","チェベット","キピエゴ","コリル","キムタイ","ベケレ","ゲブレ","ハイレ","アベバ","チェイサ","ティロル","ジルマ","レゲセ","テスファイエ","ヨハネス","アディロ","ムカンビ","オウィテ","ンガラ","キピル","チェメラ","キトニー","チェログイ","コエチ","エチュル","ジェレイ"];
function genForeign(yr){
  const talent=0.7+Math.random()*0.3,peak=[2,3,3,3,4][~~(Math.random()*5)];
  const stats={},pot={};
  SK.forEach(k=>{const v=Math.min(100,Math.max(20,Math.round(75+(Math.random()+Math.random()+Math.random()-1.5)*2*20)));stats[k]=v;pot[k]=Math.min(120,v+Math.min(Math.round((105-v)*(0.1+Math.random())),40)+5+(Math.random()>0.9?10:0));});
  const r={id:_id++,name:FOREIGN_LAST[~~(Math.random()*FOREIGN_LAST.length)]+" "+FOREIGN_NAMES[~~(Math.random()*FOREIGN_NAMES.length)],year:yr||1,talent,peak,stats,pot,condition:90,injRisk:3+~~(Math.random()*8),fatigue:0,injured:false,injTurns:0,pb5k:null,pb10k:null,pbHalf:null,trn:"bal",history:[],foreign:true};
  r.pb5k=cTrk(5000,r);r.pb10k=cTrk(10000,r);r.condition=70+~~(Math.random()*25);return r;
}
const FOREIGN_UNIVS=["山梨学院大学","東京国際大学","城西大学","専修大学","桜美林大学","国士舘大学","拓殖大学","駿河台大学","日本薬科大学","日本大学","創価大学"];
...
function genT48(str,univName){
  ...
  if(univName&&FOREIGN_UNIVS.includes(univName)){
...
function optAssign(runners,secs,eid){
  /* Foreign student limit: keep only the strongest 1 foreign runner */
...
export default function Game(){
  ...
  const startNew=useCallback(()=>{_id=0;
    ...
    setRivals(UNIV.map(([n,s,,t,m,c,rp])=>{const team=allT.find(a=>a.name===n);return{name:n,str:s,fac:{training:t,medical:m,coaching:c,reputation:rp},runners:team?team.runners:genT48(s,n),yearRes:{}};}));
  ...
  const startNextYear=useCallback(()=>{
    ...
    const newRivals=rivals.map(rv=>{const t=draftResult?draftResult.teams.find(t=>t.name===rv.name):null;
      ...
      if(FOREIGN_UNIVS.includes(rv.name)){
  ...
  useEffect(()=>{if(turn>0&&ph!=="title"){doSave({tn,tc,gameYear,turn,rs,rivals,log,fac,prestige,hakQ,zenQ,zenQTeams,hakQTeams,sRes,prevHakone,prevZennihon,prevHakoneRanks,prevZennihonRanks,recs,yearHistory,scoutHist,autoRest,autoRestThreshold,hallOfFame,teamRecords});}},[ph,turn,fac,prestige,rs,rivals,recs,scoutHist,sRes,gameYear,hallOfFame,teamRecords]);
  ...
  const runEkCommon=useCallback((isWatch)=>{const secs=ek.sec;const nR=curEk==="izumo"?10:curEk==="zennihon"?15:20;
    ...
    /* Update Hall of Fame: top 5 of all time per ekiden */
    ...
    setHallOfFame(prev=>{const n={...prev};
      ...
      /* Hakone outbound (1-5区) and inbound (6-10区) records */
      if(curEk==="hakone"&&secs.length===10){
        const outboundEnd=4;
        const out5=[...sortedRes.map(t=>({name:t.name,time:t.st[outboundEnd].cum,yr:gameYear})),...(prev.hakoneOut||[])];
  ...
  if(ph==="help")return(<><style>{CSS}</style><div style={{minHeight:"100vh",background:"var(--bg)",padding:14}}><div style={{maxWidth:700,margin:"0 auto"}} className="fi">
    ...
    <div style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:16,marginBottom:12}}>
      ...
      <div style={{fontSize:13,lineHeight:1.9,color:"var(--t2)"}}>
        <div>一部の大学（山梨学院・東京国際・城西・専修・桜美林・国士舘・拓殖・駿河台・日本薬科・日本大学）は毎年度末に40%の確率で外国人留学生を1名迎え入れます。</div>
        <div style={{marginTop:6}}>・能力は平均75・σ20と高水準</div>
  ...
  if(ph==="univ_profile"){const sel=viewRiv>=0?rivals[viewRiv]:null;if(!sel)return null;
    ...
    return(<><style>{CSS}</style><div style={{minHeight:"100vh",background:"var(--bg)",padding:14}}><div style={{maxWidth:600,margin:"0 auto"}} className="fi">
      ...
      <div style={{background:"var(--card)",borderRadius:14,padding:18,border:"1px solid var(--bdr)"}}>
        ...
        {(()=>{const ekStats={izumo:{n:"出雲",count:0,best:99,bestYr:null},zennihon:{n:"全日本",count:0,best:99,bestYr:null},hakone:{n:"箱根",count:0,best:99,bestYr:null}};
          (yearHistory||[]).forEach(yh=>{if(yh.univ&&yh.univ!==sel.name)return;});
          /* Build from scoutHist: each entry has hakone/zennihon/izumo rank */
          hist.forEach(h=>{["izumo","zennihon","hakone"].forEach(k=>{if(h[k]!=null){ekStats[k].count++;if(h[k]<ekStats[k].best){ekStats[k].best=h[k];}}});});
          /* Add current year if not in hist */
          ["izumo","zennihon","hakone"].forEach(k=>{if(yr[k]!=null){ekStats[k].count++;if(yr[k]<ekStats[k].best){ekStats[k].best=yr[k];}}});
          return(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:10}}>{["izumo","zennihon","hakone"].map(k=>{const tr=teamRecords[sel.name+":"+k];return(<div key={k} style={{padding:"6px 8px",background:"var(--bg)",borderRadius:6,fontSize:11}}><div style={{color:"var(--t2)"}}>{ekStats[k].n}</div><div style={{fontWeight:800}}>{ekStats[k].count}回 / 最高{ekStats[k].best<99?ekStats[k].best+"位":"---"}</div>{tr&&<div style={{fontSize:10,color:"var(--gold)"}}>記録 {fmt(tr.time)}</div>}</div>);})}</div>);})()}
        {(()=>{const tOut=teamRecords[sel.name+":hakoneOut"],tIn=teamRecords[sel.name+":hakoneIn"];if(!tOut&&!tIn)return null;return(<div style={{fontSize:11,color:"var(--t2)",marginBottom:10}}>箱根 往路ベスト: <b style={{fontFamily:"monospace"}}>{tOut?fmt(tOut.time):"---"}</b> / 復路ベスト: <b style={{fontFamily:"monospace"}}>{tIn?fmt(tIn.time):"---"}</b></div>);})()}
  ...
  if(ph==="hall")return(<><style>{CSS}</style><div style={{minHeight:"100vh",background:"var(--bg)",padding:14}}><div style={{maxWidth:700,margin:"0 auto"}} className="fi">
    ...
    {[{k:"hakone",l:"🏔 箱根駅伝（総合）"},{k:"hakoneOut",l:"➡ 箱根駅伝 往路（1〜5区）"},{k:"hakoneIn",l:"⬅ 箱根駅伝 復路（6〜10区）"},{k:"zennihon",l:"🌏 全日本大学駅伝"},{k:"izumo",l:"🌟 出雲駅伝"}].map(ek=>{const data=hallOfFame[ek.k]||[];return(<div key={ek.k} style={{background:"var(--card)",borderRadius:10,border:"1px solid var(--bdr)",padding:12,marginBottom:10}}>
      ...
      {data.length===0?<div style={{fontSize:11,color:"var(--t3)"}}>記録なし</div>:data.map((r,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"4px 8px",fontSize:12,borderBottom:"1px solid var(--bdr)",background:r.name===teamName?"var(--cs)":"transparent"}}><span style={{color:i===0?"var(--gold)":"var(--t)",fontWeight:i===0?800:500}}>{i+1}. {r.name}</span><span style={{fontFamily:"monospace",fontWeight:700}}>{fmt(r.time)} <span style={{fontSize:10,color:"var(--t3)"}}>(Y{r.yr})</span></span></div>))}
    </div>);})}
  </div></div></>);
  if(ph==="records"){
