...
export default function Game(){
  ...
  const startNew=useCallback(()=>{_id=0;
    ...
    setRivals(UNIV.map(([n,s,,t,m,c,rp])=>{const team=allT.find(a=>a.name===n);return{name:n,str:s,fac:{training:t,medical:m,coaching:c,reputation:rp},runners:team?team.runners:genT48(s,n,foreignAllowed),yearRes:{}};}));
  ...
  useEffect(()=>{if(turn>0&&ph!=="title"){doSave({tn,tc,gameYear,turn,rs,rivals,log,fac,prestige,hakQ,zenQ,zenQTeams,hakQTeams,sRes,prevHakone,prevZennihon,prevHakoneRanks,prevZennihonRanks,recs,yearHistory,scoutHist,autoRest,autoRestThreshold,hallOfFame,teamRecords,foreignAllowed});}},[ph,turn,fac,prestige,rs,rivals,recs,scoutHist,sRes,gameYear,hallOfFame,teamRecords,foreignAllowed]);
  ...
  if(ph==="foreign_cfg")return(<><style>{CSS}</style><div style={{minHeight:"100vh",background:"var(--bg)",padding:14}}><div style={{maxWidth:700,margin:"0 auto"}} className="fi">
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}><h2 className="sf">🌏 留学生受入設定</h2><button onClick={()=>setPh(turn>0?"settings":"title")} style={{padding:"6px 18px",borderRadius:7,background:"var(--card)",color:"var(--t)",border:"1px solid var(--bdr)"}}>← 戻る</button></div>
      ...
      {(()=>{const allYrs=new Set();["izumo","zennihon","hakone"].forEach(k=>(hallOfFame[k+"_yearly"]||[]).forEach(y=>allYrs.add(y.yr)));const yrs=[...allYrs].sort((a,b)=>b-a);if(yrs.length===0)return <div style={{fontSize:11,color:"var(--t3)"}}>年別記録なし</div>;
        return yrs.map(yr=>(<div key={yr} style={{marginBottom:10,paddingBottom:8,borderBottom:"1px solid var(--bdr)"}}>
          ...
          {[{k:"izumo",l:"出雲"},{k:"zennihon",l:"全日本"},{k:"hakone",l:"箱根"}].map(ek=>{const yEntry=(hallOfFame[ek.k+"_yearly"]||[]).find(y=>y.yr===yr);if(!yEntry)return null;return(<div key={ek.k} style={{marginLeft:8,marginBottom:4}}>
            ...
            {yEntry.top5.map((r,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:11,padding:"1px 6px",background:r.name===teamName?"var(--cs)":"transparent"}}><span style={{color:i===0?"var(--gold)":"var(--t)"}}>{i+1}. {r.name}</span><span style={{fontFamily:"monospace"}}>{fmt(r.time)}</span></div>))}
          </div>);})}
        </div>));})()}
    </div>
  </div></div></>);
