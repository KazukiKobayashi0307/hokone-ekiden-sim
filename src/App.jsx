...
function genForeign(yr){
  ...
  SK.forEach(k=>{const isElite=(k==="speed"||k==="stamina");const mean=isElite?75:55;const v=Math.min(100,Math.max(10,Math.round(mean+(Math.random()+Math.random()+Math.random()-1.5)*2*20)));stats[k]=v;const growth=Math.max(0,Math.round(_gM+(Math.random()+Math.random()+Math.random()-1.5)*2*_gS));pot[k]=Math.min(120,v+growth);});
...
const CAL={6:{t:"rec",n:"関東インカレ"},10:{t:"rec",n:"大体大記録会"},14:{t:"rec",n:"ホクレンDC"},16:{t:"half",n:"札幌ハーフ"},18:{t:"camp",n:"夏合宿"},22:{t:"rec",n:"日本インカレ"},24:{t:"zenQ",n:"全日本予選会"},26:{t:"ek",n:"出雲駅伝",eid:"izumo"},28:{t:"hakQ",n:"箱根予選会"},30:{t:"ek",n:"全日本大学駅伝",eid:"zennihon"},34:{t:"rec",n:"10000m記録挑戦会"},36:{t:"half",n:"上尾ハーフ"},37:{t:"ek",n:"箱根駅伝",eid:"hakone"}};
