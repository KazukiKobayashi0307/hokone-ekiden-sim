...
export default function Game(){
  ...
  const startNextYear=useCallback(()=>{
    ...
    /* Rival training facility fluctuation: 50% chance to change 1 of 9 skill facilities ±1 (1-10) */
    const flucRivals=newRivals.map(rv=>{if(Math.random()>=0.5)return rv;const skillKeys=["speed","stamina","stability","uphill","downhill","solo","pack","track","road"];const sk=skillKeys[~~(Math.random()*9)];const dir=Math.random()<0.5?-1:1;const ts={...(rv.fac.trainSkill||{})};const cur=ts[sk]||1;const nv=Math.max(1,Math.min(10,cur+dir));ts[sk]=nv;return{...rv,fac:{...rv.fac,trainSkill:ts}};});
